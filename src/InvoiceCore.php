...
class InvoiceCore
{
    ...
    /**
     * init config
     * @param  array  $config aisino config
    ...
    public function getContent(array $params)
    {
        ...
        foreach ($this->content_0($config) as $key => $item){
            if($item['text']!==''){
                ...
                $fpkj .= '<'.strtoupper($item['key']).'>'.$params[$item['key']].'</'.$item['key'].'>';
            }
        }
        $xm_size = count($params['items']);
        $fpkj_xm = '';
        foreach ($params['items'] as $num => $value){
            $fpkj_xm .= '<FPKJXX_XMXX>';
        ...
        foreach ($this->content_2() as $key=>$item){
            if($item['text']!==''){
                ...
                $fpkj_dd .= '<'.strtoupper($item['key']).'>'.$params[$item['key']].'</'.$item['key'].'>';
            }
        }
        $root = <<<ROOT
    ...
    public function getDownload($params)
    {
        ...
        foreach ($this->download(self::$config) as $key => $item){
            if($item['text']!==''){
                ...
                $content .= '<'.strtoupper($item['key']).'>'.$params[$item['key']].'</'.$item['key'].'>';
            }
        }
